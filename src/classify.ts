...
/** Системный промпт + накопленная подсказка по вовлечённости аудитории (см. engagement.ts). */
function buildSystemPrompt(): string {
  const hint = kvGet("engagement_hint");
  return hint ? `${SYSTEM_PROMPT}\n\nВОВЛЕЧЁННОСТЬ АУДИТОРИИ: ${hint}` : SYSTEM_PROMPT;
}
