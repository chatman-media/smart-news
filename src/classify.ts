...
import { type Channel, kvGet } from "./db";
...
const SYSTEM_PROMPT = (
  focus: string,
) => `Ты — редактор новостного Telegram-канала. Аудитория: ${focus}. Твоя задача — отфильтровать информационный шум и оставить только действительно полезное для этой аудитории.
...
- короткие видео-посты (рилсы) с атмосферным локальным контентом: закаты, море, природа, животные, стрит-фуд, уличные сценки, красивые места — категория vibes, tone positive; заголовок придумай сам по подписи, summary — одно лёгкое предложение без канцелярита
...
- шок-видео и рилсы с негативом: аварии, драки, скандалы с туристами
...
/** Системный промпт канала + накопленная подсказка по вовлечённости его аудитории (см. engagement.ts). */
function buildSystemPrompt(channel: Channel): string {
  const base = SYSTEM_PROMPT(channel.focus);
  const hint = kvGet(`engagement_hint:${channel.id}`);
  return hint ? `${base}\n\nВОВЛЕЧЁННОСТЬ АУДИТОРИИ: ${hint}` : base;
}

export async function classify(channel: Channel, postText: string): Promise<Verdict> {
...
const TRIAGE_SYSTEM = (focus: string) =>
  `Ты — быстрый триаж-фильтр новостного канала. Аудитория: ${focus}. По заголовкам отбери те, что МОГУТ быть полезны этой аудитории: визы/законы, безопасность и предупреждения, транспорт, деньги, инфраструктура (вода/свет/интернет), события, погодные угрозы, практичные локальные новости. Видео-посты (помечены 🎬) с атмосферным контентом региона — закаты, природа, еда, сценки — тоже бери. Отбрасывай очевидный шум: мемы, рекламу, криминальную хронику без пользы, политические срачи, новости без связи с регионом аудитории, спорт, гороскопы. Сомневаешься — бери: дальше полный фильтр разберётся.`;

/** Дешёвый батч-отбор: какие из заголовков стоят полной классификации. */
export async function triage(
  channel: Channel,
  items: { index: number; headline: string }[],
): Promise<Set<number>> {
  ...
  const response = await llm.chat.completions.create({
    ...
    messages: [
      { role: "system", content: TRIAGE_SYSTEM(channel.focus) },
